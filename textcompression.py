...
input_label.grid(row=0, column=0, sticky=(tk.W, tk.N))

input_text = ScrolledText(input_frame, height=5, width=50)
input_text.grid(row=1, column=0, columnspan=2, padx=5, pady=5)

compress_button = ttk.Button(input_frame, text="Compress", command=compress_text)
compress_button.grid(row=2, column=0, columnspan=2, pady=5)
...
codes_text = ScrolledText(output_frame, height=5, width=50)
